...
<!--
  ...
  Windhawk only permits images hosted on:
      https://i.imgur.com
      https://raw.githubusercontent.com

  To finish this section:
    1. Upload each image below to Imgur or a public GitHub repo
    2. Copy the DIRECT link (must end in .png / .jpg / .gif)
    3. Replace the REPLACE_ME URL on the matching line
    4. Delete this comment block

  Local file -> line mapping:
    Tourne_Table_Visualizer_Showcase_3__7_.png    -> Desktop overview
    Tourne_Table_Visualizer_Showcase_21_08_03_58__2_.png -> Oscilloscope closeup
    Tourne_Table_Visualizer_Showcase_3.png        -> Taskbar strip
    Screenshot_2026-09-10_162114.png              -> Settings panel
  ==========================================================================
-->

![Tourne'Table Audio Visualizer running on the desktop](https://i.imgur.com/REPLACE_ME.png)

*Oscilloscope shape, middle anchor, blurred background panel with a single-pixel border.*

![Oscilloscope waveform closeup](https://i.imgur.com/REPLACE_ME.png)

*Closeup: live waveform trace rendered as a continuous line.*

![Visualizer in the taskbar strip](https://i.imgur.com/REPLACE_ME.png)

*Positioned along the bottom of the screen at 99% vertical position.*
...
- **Oscilloscope** - Traces the live raw waveform as a continuous line, with an optional multiband coloring mode that tints the trace by whichever part of the spectrum (low/mid/high) is currently dominant
...
- **Bar Count:** 1–2048 (enough to span a 4K or ultrawide display at minimum bar width)
- **Thickness**
- **Gap**
- **Maximum Size**
- **Idle Size**
...
- **Configurable FFT Size** - 1024 / 2048 / 4096 / 8192, trading a bit more CPU for finer frequency detail
- **Frequency Scale** - Log (default, most natural), Linear (even Hz spacing), or Mel (perceptual pitch spacing)

### Peak Frequency Readout

Optional numeric readout showing the current dominant frequency (e.g. `1.2 kHz`). Reuses the same text rendering, font and colour as the Now Playing Display.

Its position is configurable on both axes:

- **Horizontal:** Left, Center, or Right
- **Vertical:** Above, Top, Middle, Bottom, or Below

**Above** and **Below** place the readout fully clear of the bars or waveform, so it never overlaps the visualization itself. The render surface reserves room for whichever placement is selected.
...
### Pre-Rendered Background Blur

A Gaussian blur is a full-image convolution and by far the most expensive operation in the scene, but its input — the wallpaper — does not change between frames.

The blur is now **evaluated exactly once** into a cached bitmap, then simply blitted each frame. Once baked, both the effect graph and the full-size source wallpaper bitmap are released, **reducing memory as well as per-frame GPU work**.

Profiling with Windows Performance Analyzer showed the previous per-frame blur evaluation dominating Direct2D time whenever background blur was enabled.

The cached result covers only the widget's bounding box rather than the whole desktop. The blur still samples the full-size wallpaper while being computed, since a Gaussian reads pixels from beyond the box's edges, but only the box-sized result is retained - replacing roughly 8 MB of video memory at 1080p with tens of KB. It is re-baked automatically whenever the box moves or resizes.

### Pause When Covered

The visualizer draws on the desktop layer, behind icons. Any maximized window hides it completely — yet without this check, the full render path keeps running at the target frame rate drawing pixels **nobody can see**.

When the visualizer's region is fully covered by another window, rendering and audio capture both stop until it becomes visible again.

- Evaluated once per second, not per frame
- Deliberately conservative: only **full** containment counts, so partial overlap never causes the visualizer to disappear while still partly visible

### Widget-Sized Swap Chain

The render surface previously spanned the **entire desktop**, even though the visualizer occupies a thin strip of it. Every frame cleared and presented millions of untouched pixels, and two full-desktop buffers sat in video memory for the life of the overlay.

The swap chain is now sized to the widget's **bounding box only**, with the composition visual offset to position it. Drawing happens in local coordinates relative to that box.

At 1080p with a typical bar strip, this reduces the render surface by roughly **an order of magnitude** in both per-frame pixel work and VRAM. It resizes automatically when settings, DPI, or display arrangement change.

A small margin is included so stroked shapes and the radial spokes cannot clip at the edges.

### Reduced Frame Latency

DXGI queues up to three frames ahead by default. For a passive desktop widget that is **pure latency and power draw with no benefit**, since the widget is never GPU-bound.

The maximum frame latency is now capped at **1**.
...
Optionally fades the entire visualizer out during prolonged silence instead of leaving it rendered at a reduced refresh rate.

Once the fade reaches full transparency, **rendering stops completely** — a single blank frame is presented to clear the display, after which the render path exits immediately until audio returns. Nothing is drawn and nothing is presented while hidden.
