...
// @version             1.1.0
// @author              USER-TOURNE [Fix & Fork of Salyts "Desktop-Audio-Visualizer"]
    ...
    while (g_captureRunning.load(std::memory_order_relaxed)) {
        ...
        while (ringCount >= VIZ_FFT_SIZE) {
            ...
            if (g_settings.shape == VizShape::Oscilloscope) {
                int wstep = std::max(1, VIZ_FFT_SIZE / VIZ_WAVE_SAMPLES);
        ...
        if (g_settings.nowPlayingEnabled && g_dwriteTextFormat && g_nowPlayingBrush) {
            ...
            if (npAlpha > 0.01f) {
                ...
                if (!displayText.empty()) {
                    ...
                    D2D1_RECT_F npRect = D2D1::RectF(blockX - 100.0f, blockY - npHeight - npMargin,
                                                     blockX + totalWidth + 100.0f, blockY - npMargin);
